#!/usr/bin/env python3
"""
Main GUI for Ackermann Rover Control - Refactored Version
Imports modular components instead of having everything in one file
"""
...
class RoverControlGUI(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Rover GPS Navigation Control - Refactored")
        ...
        self.navigator = WaypointNavigator(waypoint_tolerance=0.1)  # Change from 2 to 0.5 meters
        ...
        self.autonomous_mode = False  # NEW: Track if in autonomous mode
...
Safety: 2s auto-stop timeout""")
    ...
    def add_gps_controls(self, layout):
        """Add GPS control buttons to layout"""
        gps_group = QGroupBox("GPS Control")
        gps_layout = QVBoxLayout()
        
        self.start_gps_btn = QPushButton("Start GPS")
        self.start_gps_btn.clicked.connect(self.start_gps_node)
        self.start_gps_btn.setStyleSheet("background-color: #007bff; color: white; padding: 8px;")
        gps_layout.addWidget(self.start_gps_btn)
        
        self.stop_gps_btn = QPushButton("Stop GPS")
        self.stop_gps_btn.clicked.connect(self.stop_gps_node)
        self.stop_gps_btn.setStyleSheet("background-color: #6c757d; color: white; padding: 8px;")
        gps_layout.addWidget(self.stop_gps_btn)
        
        self.gps_status_label = QLabel("GPS: Not started")
        self.gps_status_label.setStyleSheet("background-color: #fff3cd; padding: 5px; border-radius: 3px;")
        gps_layout.addWidget(self.gps_status_label)
        
        gps_group.setLayout(gps_layout)
        layout.addWidget(gps_group)
    
    def add_lidar_safety_controls(self, layout):
        """Add LiDAR safety controls to layout"""
        lidar_group = QGroupBox("LiDAR Safety")
        lidar_layout = QVBoxLayout()
        
        # Enable/disable toggle
        self.lidar_safety_checkbox = QCheckBox("Enable LiDAR Safety (Stop on obstacles)")
        self.lidar_safety_checkbox.setChecked(True)
        self.lidar_safety_checkbox.setToolTip("When enabled, rover stops automatically if obstacles detected")
        lidar_layout.addWidget(self.lidar_safety_checkbox)
        ...
        self.safety_distance_slider = QSlider(Qt.Horizontal)
        self.safety_distance_slider.setRange(30, 150)  # 0.3m to 1.5m
        self.safety_distance_slider.setValue(50)  # 0.5m default
        self.safety_distance_slider.valueChanged.connect(
            lambda v: self.update_safety_distance(v, distance_label)
        )
        lidar_layout.addWidget(self.safety_distance_slider)
    ...
    def ros_worker(self):
        ...
        try:
            ...
            # Create ROS node with LiDAR safety enabled
    ...
    def check_gps_data(self):
        ...
        try:
            ...
            # Fallback: show waiting status if GPS not active
    ...
    def process_keyboard_input(self):
        ...
        # Don't process keyboard if in autonomous mode
    ...
    def start_gps_node(self):
        ...
        try:
            ...
            self.gps_status_label.setText(f"GPS: Error - {str(e)[:30]}")
            self.gps_status_label.setStyleSheet("background-color: #f8d7da; padding: 5px; border-radius: 3px;")
            self.start_gps_btn.setEnabled(True)
            self.stop_gps_btn.setEnabled(False)
    ...
    # NEW: Autonomous Navigation Functions
    ...
    def start_autonomous_navigation(self):
        ...
        print(f"DEBUG: GPS status check - lat={self.last_gps_lat}, lon={self.last_gps_lon}, status={self.gps_status}")
        
        if self.last_gps_lat == 0.0 or self.last_gps_lon == 0.0:
            QMessageBox.warning(
                self, "No GPS Data",
                "Cannot start navigation without valid GPS signal.\n"
                f"Current GPS: {self.last_gps_lat:.6f}, {self.last_gps_lon:.6f}\n\n"
                "Please ensure GPS is started and has a fix."
            )
            return
        ...
        if self.navigator.start_navigation():
            ...
            print(f"Starting position: {self.last_gps_lat:.6f}, {self.last_gps_lon:.6f}")
        else:
    ...
    def update_navigation(self):
        ...
        if self.gps_status < 0 or self.last_gps_lat == 0.0:
            print("DEBUG: No GPS data for navigation")
            return
        
        print(f"DEBUG: Current GPS: {self.last_gps_lat:.6f}, {self.last_gps_lon:.6f}")

        # Get current heading from IMU (if available)
        current_heading = None  # TODO: Get from IMU when integrated
        ...
        if self.ros_node and not reached:
            print(f"DEBUG: Sending command to rover")
            self.send_movement_command(linear, angular)
        elif reached:
        ...
        if target:
            print(f"Navigating to waypoint {target['index']+1}/{target['total']}: "
    ...
    def update_safety_distance(self, value, label):
        ...
        distance = value / 100.0  # Convert to meters
    ...
    def update_lidar_status(self):
        ...
        if not self.ros_node:
            self.obstacle_status_label.setText("ROS node not ready")
            return
        
        if not hasattr(self.ros_node, 'get_lidar_safety_info'):
            self.obstacle_status_label.setText("LiDAR safety not available")
            return
        
        # Toggle LiDAR safety based on checkbox
        if hasattr(self.ros_node, 'enable_lidar_safety'):
        ...
        try:
            ...
            import random
            if random.random() < 0.05:  # 5% chance = ~every 4 seconds
                print(f"LiDAR Status: {obstacle_info}")
            ...
            self.obstacle_status_label.setText(f"Error: {str(e)[:30]}")
            print(f"LiDAR status update error: {e}")
...
def main():
    ...
    try:
        ...
        print("Rover Control GUI started - Refactored version")
        print("Use W/A/S/D or joystick to control")
