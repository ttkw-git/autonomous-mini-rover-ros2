#!/usr/bin/env python3
"""
GPS Waypoint Recording Widget - Simple Version for Integration
Add this to your existing rover control GUI
"""
...
class GPSWaypointWidget(QWidget):
    ...
    def start_recording(self):
        ...
        if not self.parent_gui or not hasattr(self.parent_gui, 'direct_gps'):
            QMessageBox.warning(self, "No GPS", "Direct GPS not available. Please ensure GPS is connected.")
            return
         

        gps_data = self.parent_gui.direct_gps.get_gps_data()
        if not gps_data['available'] or gps_data['lat'] == 0.0 or gps_data['lon'] == 0.0:
            QMessageBox.warning(self, "No GPS Fix", "No GPS fix available. Please wait for GPS signal.")
            return
               
        # Set GPS origin if not set
        if not self.origin_set:
            gps_data = self.parent_gui.direct_gps.get_gps_data()
            if gps_data['available'] and gps_data['lat'] != 0.0 and gps_data['lon'] != 0.0:
                self.origin_lat = gps_data['lat']
                self.origin_lon = gps_data['lon']
                self.origin_set = True
                print(f"GPS origin set: {self.origin_lat:.6f}, {self.origin_lon:.6f}")
            else:
                QMessageBox.warning(self, "GPS Origin", "Cannot set GPS origin. Please wait for valid GPS fix.")
                return
        ...
        print("Started GPS waypoint recording")
    ...
    def record_waypoint(self):
        ...
        if not self.recording or not self.parent_gui or not hasattr(self.parent_gui, 'direct_gps'):
            return
    ...
    def view_waypoints(self):
        ...
        if len(self.waypoints) > 1:
            ...
            summary = QLabel(f"Total waypoints: {len(self.waypoints)}\nApproximate path length: {total_distance:.1f} meters")
        else:
            summary = QLabel(f"Total waypoints: {len(self.waypoints)}")
        ...
        for wp in self.waypoints:
            item_text = f"#{wp['id']}: {wp['datetime']} - GPS({wp['latitude']:.6f}, {wp['longitude']:.6f}) - XY({wp['x']:.1f}, {wp['y']:.1f})"
